...
| 4:15 | Dashboard: click bounty **#11** (Disputed → ruling → Refunded); optionally the committee contract on Etherscan | "Since then I replaced the single arbiter with a staked committee. On this bounty nobody picked the judges: the next block's hash drew three staked jurors, the buyer handed them the key, each reran the tasks and voted blind, and the tally ruled unanimously for the buyer. A juror who votes against the reproducible answer loses stake." |
